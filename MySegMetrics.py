...
def DiceCoefficient(pre, target, **args):
    dim = len(target.shape)
    sum_axis = tuple(np.arange(dim - 1))
...
def JaccardCoefficient(pre, target, **args):
    dim = len(target.shape)
    inter = pre * target
    union = pre + target
...
def Recall(pre, target, tp = None, tn = None, fp = None, fn = None, **args):
    if tp is None:
...
#Specificity TNR
def Specificity(pre, target, tp = None, tn = None, fp = None, fn = None, **args):
    if tn is None:
...
#Precision PPV
def Precision(pre, target, tp = None, tn = None, fp = None, fn = None, **args):
    if tp is None:
        tp = get_TP(pre, target)
    if fp is None:
        fp = get_FP(pre, target)
...
#Fallout FPR
def FPR(pre, target, tp = None, tn = None, fp = None, fn = None, **args):
    return 1 - Specificity(pre, target, tn, fp)

def FNR(pre, target, tp = None, tn = None, fp = None, fn = None, **args):
    return 1 - Recall(pre, target, tp, fn)

def Fbeta_Score(pre, target, beta,
            tp = None, tn = None, fp = None, fn = None, **args
            ):
    ppv = Precision(pre, target, tp, fp)
    tpr = Recall(pre, target, tp, fn)
    score = ( math.pow(beta, 2) + 1 ) * ppv * tpr / \
        ( math.pow(beta, 2) * ppv + tpr + eps )
    return score

def F1Score(pre, target, tp = None, tn = None, fp = None, fn = None, **args):
    return Fbeta_Score(pre, target, 1, tp, fp, fn)

def ConsistencyError(pre, target, **args):
    dim = len(target.shape)
    sum_axis = tuple(np.arange(dim - 1))
    diff = pre - target
...
def GlobalConsistencyError(pre, target, **args):
    dim = len(target.shape)
    sum_axis = tuple(np.arange(dim - 1))
    diff = pre - target
    diff[ np.where(diff != 0) ] = 1
...
def VolumetricDistance(pre, target, **args):
    dim = len(target.shape)
    sum_axis = tuple(np.arange(dim - 1))
...
def VolumtricSimilarity(pre, target, **args):
...
def InterclassCorrelation(pre, target, model = 'one-way', type = 'single', absolute = True, **args):
    n = target.size / target.shape[-1]
    k = 2
...
def ProbabilisticDistance(pre, target, **args):
    dim = len(target.shape)
    sum_axis = tuple(np.arange(dim - 1))
    score = np.abs( target - pre ).sum(sum_axis) / ( 2 * (pre * target).sum(sum_axis) + eps )
    return score

def Kappa(pre, target, tp = None, tn = None, fp = None, fn = None, **args):
    n = target.size / target.shape[-1]
    if tp is None:
        tp = get_TP(pre, target)
    if fp is None:
...
def HausdorffDistance(pre, target, length_pre = None, length_target = None, **args):
    distance = []
    dim = len(pre.shape)
    if dim == 3:
        mode = '2d'
    ...
    if mode == '2d':
        for i in range(length_pre.shape[0]):
            ...
            if np.sum(length_target[i]) != 0:
                ...
                d = np.sqrt( np.square( target_cord - pre_cord ).sum(2) + 1e-5 )
                dpt = d.min(1)
                dtp = d.min(0)
...
def HausdorffDistancePercent(pre, target, percent, length_pre = None, length_target = None, **args):
    distance = []
    dim = len(pre.shape)
    if dim == 3:
        mode = '2d'
...
def HausdorffDistance95(pre, target, length_pre = None, length_target = None, **args):
    return HausdorffDistancePercent(pre, target, 95, length_pre, length_target)

def AverageHausdorffDistance(pre, target, length_pre = None, length_target = None, **args):
    distance = []
    dim = len(pre.shape)
    if dim == 3:
        mode = '2d'
...
def AverageSurfaceDistance(pre, target, length_pre = None, length_target = None, **args):
    distance = []
    dim = len(pre.shape)
    if dim == 3:
        mode = '2d'
...
def AverageSymmetricSurfaceDistance(pre, target, length_pre = None, length_target = None, **args):
    distance = []
    dim = len(pre.shape)
    if dim == 3:
        mode = '2d'
...
def debug():
    ...
    cmatrix = ConfusionMatrix(pre, target)
    tp = get_TP(pre, target, cmatrix)
    tn = get_TN(pre, target, cmatrix)
    fp = get_FP(pre, target, cmatrix)
    fn = get_FN(pre, target, cmatrix)
    ...
    if dim == 3:
        ...
        mode = '3d'
    # length_pre, length_tar = SobelOp(pre, target, mode)
    
    dsc = DiceCoefficient(pre, target)
    metrics['dsc'] = dsc
    jc = JaccardCoefficient(pre, target)
    metrics['jac'] = jc
    sen = Recall(pre, target, tp, tn, fp, fn)
    metrics['sen'] = sen
    spe = Specificity(pre, target, tp, tn, fp, fn)
    metrics['spe'] = spe
    fpr = FPR(pre, target, tp, tn, fp, fn)
    metrics['fpr'] = fpr
    fnr = FNR(pre, target, tp, tn, fp, fn)
    metrics['fnr'] = fnr
    ppv = Precision(pre, target, tp, tn, fp, fn)
    metrics['ppv'] = ppv
    f1 = F1Score(pre, target, tp, tn, fp, fn)
    metrics['f1'] = f1
    ...
    metrics['vs'] = vs
    mi = MutualInformation(pre, target, tp, tn, fp, fn)
    metrics['mi'] = mi
    voi = VariationofInformation(pre, target, tp, tn, fp, fn)
    metrics['voi'] = voi
    icc = InterclassCorrelation(pre, target)
    metrics['icc'] = icc
    pbd = ProbabilisticDistance(pre, target)
    metrics['pbd'] = pbd
    kappa = Kappa(pre, target, tp, tn, fp ,fn)
    metrics['kappa'] = kappa
    # hd = HausdorffDistance(pre, target, length_pre, length_tar)
    # metrics['hd'] = hd
    # hd95 = HausdorffDistancePercent(pre, target, 95, length_pre, length_tar)
    # metrics['hd95'] = hd95
    # avd = AverageHausdorffDistance(pre, target, length_pre, length_tar)
    # metrics['avd'] = avd
    # assd = AverageSymmetricSurfaceDistance(pre, target, length_pre, length_tar)
    # metrics['assd'] = assd
