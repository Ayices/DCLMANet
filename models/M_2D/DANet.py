...
class DANet(ResNet50):
    ...
    def __init__(self, nclass = 2, aux=True, **kwargs):
    ...
    def forward(self, x):
        ...
        if self.aux:
            ...
            out = torch.mean(torch.cat((x0.unsqueeze(-1), x1.unsqueeze(-1), x2.unsqueeze(-1)), -1), dim = -1)
...
def get_danet(num_classes=2, backbone='resnet50', pretrained_base=True, **kwargs):
    model = DANet(num_classes, backbone=backbone, pretrained_base=pretrained_base, **kwargs)
    return model
...
###########################################################################
    ...
    net = get_danet(num_classes=5).cuda()
    out = net(img)
    print(out.shape)
