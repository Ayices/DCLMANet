...
class FCN32s(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.Softmax_layer(score)  # size=(N, n_class, x.H/1, x.W/1)
...
def get_FCN(name = 'FCN32s', n_class = 2):
    vgg_model = VGGNet(requires_grad=True)
    if name == 'FCN32s':
        return FCN32s(pretrained_net=vgg_model, n_class=n_class)
    elif name == 'FCN16s':
        return FCN16s(pretrained_net=vgg_model, n_class=n_class)
    elif name == 'FCN8s':
...
if __name__ == "__main__":
    ...
    net = VGGNet(requires_grad=True).cuda()
    out = net(img)
    print(out['x5'].shape)
    ...
    net = FCNs(pretrained_net=vgg_model, n_class=5).cuda()
    out = net(img)
    print(out.shape)
