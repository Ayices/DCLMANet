...
class DoublePyConv(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.pyconv = nn.Sequential(
            ...
            PyConvBlock(in_ch, out_ch // 4, pyconv_kernels=[3, 5, 7, 9], pyconv_groups=[1, 4,8,16]),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True),
            PyConvBlock(out_ch,out_ch // 4,pyconv_kernels=[3,5,7,9],pyconv_groups=[1,4,8,16]),
...
###########################################################################
    ...
    net = PyConvUnet(in_ch=3, out_ch=5).cuda()
    out = net(img)
    print(out.shape)