...
from module.layers import unetConv2, unetUp_origin

class UNet2Plus(nn.Module):
    def __init__(self, n_channels=1, n_classes=2, bilinear=True, feature_scale=4, 
                 is_deconv=False, is_batchnorm=True, is_ds=False):
...
#                 init_weights(m, init_type='kaiming')
    ...
    def forward(self, inputs):
        ...
        if self.is_ds:
            ...
            return F.softmax(final_4, 1)


if __name__ == '__main__':
    # models = UNet2Plus()
    # print('# generator parameters:', 1.0 * sum(param.numel() for param in models.parameters()) / 1000000)
    #
    # params = list(models.named_parameters())
    ...
    net = UNet2Plus(n_channels=3, n_classes=5).cuda()
    out = net(img)
    print(out.shape)