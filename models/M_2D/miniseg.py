...
class MiniSeg(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.aux:
            return (self.Softmax_layer(pred1), self.Softmax_layer(pred2),
                    self.Softmax_layer(pred3), self.Softmax_layer(pred4), )
        else:
            return self.Softmax_layer(pred1)
...
if __name__ == '__main__':
    ...
    net = MiniSeg(in_input=3, classes=5).cuda()
    out = net(img)
    print(out.shape)