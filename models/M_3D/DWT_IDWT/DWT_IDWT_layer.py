...
class IDWT_3D_Groups(Module):
    ...
    def __init__(self, wavename):
        ...
        :param band_high: 小波重构所用高频滤波器组
        """
        super(IDWT_3D_Groups, self).__init__()
        wavelet = pywt.Wavelet(wavename)
        self.band_low = wavelet.dec_lo
        self.band_high = wavelet.dec_hi
        self.band_low.reverse()
    ...
    def forward(self, input):
        features_size = input.size()
        STEP = features_size[1]//8
        LLL = input[:, 0:STEP, :, :]
        LLH = input[:, STEP:2*STEP, :, :]
        LHL = input[:, 2*STEP:3*STEP, :, :]
        LHH = input[:, 3*STEP:4*STEP, :, :]
        HLL = input[:, 4*STEP:5*STEP, :, :]
        HLH = input[:, 5*STEP:6*STEP, :, :]
        HHL = input[:, 6*STEP:7*STEP, :, :]
        HHH = input[:, 7*STEP:8*STEP, :, :]
        assert len(LLL.size()) == len(LLH.size()) == len(LHL.size()) == len(LHH.size()) == 5
        assert len(HLL.size()) == len(HLH.size()) == len(HHL.size()) == len(HHH.size()) == 5
        self.input_depth = LLL.size()[-3] + HHH.size()[-3]
        self.input_height = LLL.size()[-2] + HHH.size()[-2]
