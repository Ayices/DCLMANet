...
class MixedFusion_Block(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, act_fn):
        ...
        self.layer1 = nn.Sequential(nn.Conv3d(in_dim * 3, in_dim, kernel_size=3, stride=1, padding=1),
                                    nn.BatchNorm3d(in_dim), act_fn, )

        # revised in 09/09/2019.
        # self.layer1 = nn.Sequential(nn.Conv3d(in_dim*3, in_dim,  kernel_size=1),nn.BatchNorm3d(in_dim),act_fn,)
        self.layer2 = nn.Sequential(nn.Conv3d(in_dim * 2, out_dim, kernel_size=3, stride=1, padding=1),
                                    nn.BatchNorm3d(out_dim), act_fn, )
    ...
    def forward(self, x1, x2, xx):
        ...
        modal_in1 = torch.reshape(x1, [x1.shape[0], 1, x1.shape[1], x1.shape[2], x1.shape[3], x1.shape[4]])
        modal_in2 = torch.reshape(x2, [x2.shape[0], 1, x2.shape[1], x2.shape[2], x2.shape[3], x1.shape[4]])
...
class MixedFusion_Block0(nn.Module):
    def __init__(self, in_dim, out_dim, act_fn):
        ...
        self.layer1 = nn.Sequential(nn.Conv3d(in_dim * 3, in_dim, kernel_size=3, stride=1, padding=1),
                                    nn.BatchNorm3d(in_dim), act_fn, )
        # self.layer1 = nn.Sequential(nn.Conv3d(in_dim*3, in_dim, kernel_size=1),nn.BatchNorm3d(in_dim),act_fn,)
        self.layer2 = nn.Sequential(nn.Conv3d(in_dim, out_dim, kernel_size=3, stride=1, padding=1),
                                    nn.BatchNorm3d(out_dim), act_fn, )
    ...
    def forward(self, x1, x2):
        ...
        modal_in1 = torch.reshape(x1, [x1.shape[0], 1, x1.shape[1], x1.shape[2], x1.shape[3], x1.shape[4]])
        modal_in2 = torch.reshape(x2, [x2.shape[0], 1, x2.shape[1], x2.shape[2], x2.shape[3], x2.shape[4]])
        modal_cat = torch.cat((modal_in1, modal_in2), dim=1)
        fusion_max = modal_cat.max(dim=1)[0]
...
class Multi_modal_generator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf):
        ...
        self.down_1_0 = nn.Sequential(
            nn.Conv3d(in_channels=self.in_dim, out_channels=self.out_dim, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm3d(self.out_dim), act_fn,
            nn.Conv3d(in_channels=self.out_dim, out_channels=self.out_dim, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm3d(self.out_dim), act_fn,
        )
        self.pool_1_0 = maxpool()

        self.down_2_0 = nn.Sequential(
            nn.Conv3d(in_channels=self.out_dim, out_channels=self.out_dim * 2, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm3d(self.out_dim * 2), act_fn,
            nn.Conv3d(in_channels=self.out_dim * 2, out_channels=self.out_dim * 2, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm3d(self.out_dim * 2), act_fn,
        )
        self.pool_2_0 = maxpool()

        self.down_3_0 = nn.Sequential(
            nn.Conv3d(in_channels=self.out_dim * 2, out_channels=self.out_dim * 4, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm3d(self.out_dim * 4), act_fn,
            nn.Conv3d(in_channels=self.out_dim * 4, out_channels=self.out_dim * 4, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm3d(self.out_dim * 4), act_fn,
        )
        ...
        self.down_fu_4 = nn.Sequential(
            nn.Conv3d(in_channels=self.out_dim * 4, out_channels=self.out_dim * 8, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm3d(self.out_dim * 8), act_fn, )
        ...
        self.out = nn.Sequential(nn.Conv3d(int(self.out_dim), 1, kernel_size=3, stride=1, padding=1),
                                 nn.Tanh())  # self.final_out_dim
        ...
        self.out1 = nn.Sequential(nn.Conv3d(int(self.out_dim), 1, kernel_size=3, stride=1, padding=1),
                                  nn.Tanh())  # self.final_out_dim
        ...
        self.out2 = nn.Sequential(nn.Conv3d(int(self.out_dim), 1, kernel_size=3, stride=1, padding=1),
                                  nn.Tanh())  # self.final_out_dim
...
if __name__ == '__main__':
    ...
    img = torch.randn(2, 2, 64, 64, 64).cuda()
    generator = Multi_modal_generator(input_nc=1, output_nc=1, ngf=32).cuda()
    out_generator = generator(img)
    print(out_generator[0].shape)
    print(out_generator[1].shape)
    print(out_generator[2].shape)
    discrimator = Discriminator(in_channels=1).cuda()
    out = discrimator(out_generator[0])
    print(out.shape)
