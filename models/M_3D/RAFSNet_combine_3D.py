...
class basicNet(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        return out_put, output_1_U_C, self.Softmax_layer(output)


class RAFSNet_combine(torch.nn.Module):
    def __init__(self,
                 in_channels=1,
                 num_class=2,
                 channel_width=8,
        ...
        super(RAFSNet_combine, self).__init__()
        ...
        self.cov_final_fusion = nn.Conv3d(in_channels= (multimodal_num+1) * channel_width, out_channels=num_class, kernel_size=1)
    ...
    def forward(self, in_list):
        ...
        output_U_C_list = []
        for basicNet_num in range(self.multimodal_num):
            basic_features, output_U_C, basic_out = self.basicNets[basicNet_num](in_list[:, basicNet_num, :, :, :].view(b, 1, w, h, l))
            ...
            output_U_C_list.append(output_U_C)
        ...
        output_1_U = self.upsampling_1(output_2_U_C, LLH_1, LHL_1, LHH_1, HLL_1, HLH_1, HHL_1, HHH_1)
        output_1_U_C = self.cov3d_12_de(self.cov3d_11_de(torch.cat((output_1_U, output_fusion_1_D), dim = 1)))

        output_fusion = self.cov_final(output_1_U_C)
        basic_out_list.append(self.Softmax_layer(output_fusion))

        output_U_C_list.append(output_1_U_C)
        output_U_C_all = torch.stack(output_U_C_list, 1)
        output_U_C_all = output_U_C_all.view(output_U_C_all.size(0),
                                             output_U_C_all.size(1)*output_U_C_all.size(2),
                                             output_U_C_all.size(3),
                                             output_U_C_all.size(4),
                                             output_U_C_all.size(5))
        output = self.cov_final_fusion(output_U_C_all)
...
if __name__ == '__main__':
    ...
    model = RAFSNet_combine(in_channels=1, num_class=1, channel_width=8, multimodal_num=2).cuda()
    print('====================================')
    start = datetime.now()
