...
from module.layers_3D import unetConv2
...
class UNet3Plus(nn.Module):
    ...
    def forward(self, inputs):
        ...
        hd2 = self.relu3d_1(self.bn3d_1(self.conv3d_1(torch.cat((h1_PT_hd2, h2_Cat_hd2, hd3_UT_hd2, hd4_UT_hd2, hd5_UT_hd2), 1)))) # hd2->160*160*UpChannels
...
###########################################################################
    ...
    # net = UNet3Plus(n_channels=3, n_classes=5).cuda()
    # out = net(img)
    # print(out.shape)
    ...
    # device = torch.device("cuda" if use_cuda else "cpu")
    # img = torch.randn(2, 3, 128, 128).cuda()
    # net = UNet3Plus_DeepSup(n_channels=3, n_classes=5).cuda()
    # out = net(img)
    # print(out[0].shape)
    # print(out[1].shape)
    # print(out[2].shape)
    ...
    # net = UNet3Plus_DeepSup_CGM(n_channels=2, n_classes=2).cuda()
    # out = net(img)
    # print(out.shape)


    # img = torch.randn(2, 2, 64, 64, 64)
    # net = UNet3Plus_DeepSup_CGM(n_channels=2, n_classes=2)
    # out = net(img)
    # print(out.shape)

    use_cuda = torch.cuda.is_available()
    device = torch.device("cuda" if use_cuda else "cpu")
    img = torch.randn(2, 3, 64, 64, 64).cuda()
    net = UNet3Plus_DeepSup(n_channels=3, n_classes=5).cuda()
    out = net(img)
    print(out[0].shape)
    print(out[1].shape)
